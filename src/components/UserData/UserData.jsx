...
const UserData = ({ domains, isTech }) => {
            ...
            const doc = {
                ...
                header: () => ({
                    columns: [
                        ...
                        {
                            text: moment(Date.now()).format("DD.MM.Y HH:mm"),
                            alignment: "right",
                        },
                    ],
                    margin: [40, 30, 40, 0],
                ...
                content: [
                    ...chunk.map((item, i) => {
                        let domainData = [
                            ...
                            item.registered_at
                                ? [
                                      {
                                          text: formatMessage({
                                              id: "domain.registered_at",
                                          }),
                                          bold: true,
                                      },
                                      moment(item.registered_at).format(
                                          "DD.MM.Y HH:mm"
                                      ),
                                  ]
                                : [],
                            ...
                            item.outzone_at
                                ? [
                                      {
                                          text: formatMessage({
                                              id: "domain.outzone_at",
                                          }),
                                          bold: true,
                                      },
                                      moment(item.outzone_at).format(
                                          "DD.MM.Y HH:mm"
                                      ),
                                  ]
                                : [],
                            ...
                            item.force_delete_at
                                ? [
                                      {
                                          text: formatMessage({
                                              id: "domain.force_delete_at",
                                          }),
                                          bold: true,
                                      },
                                      moment(item.force_delete_at).format(
                                          "DD.MM.Y HH:mm"
                                      ),
                                  ]
                                : [],
                        ...
                        return [
                            ...
                            },
                            {
                                ...
                                layout: {
                                    hLineWidth: (lineIndex, node) =>
                                        lineIndex === 0 ||
                                        lineIndex === node.table.body.length
                                            ? 1
                                            : 1,
                                    vLineWidth: () => 0,
                                    hLineColor: () => "#eaebed",
                            ...
                            {
                                text: formatMessage({ id: "domain.statuses" }),
                                style: "subheader",
                            },
                            {
                                style: "table",
    ...
    const handleCsvData = async () => {
        ...
        const csv = domains.map((item) => {
            return {
                [formatMessage({ id: "domain.name" })]: item.name
                    ? item.name
                    : "",
                [formatMessage({ id: "domain.transfer_code" })]:
                    item.transfer_code ? item.transfer_code : "",
                [formatMessage({ id: "domain.registrar" })]: item.registrar.name
                    ? `${item.registrar.name} (${item.registrar.website})`
                    : "",
                [formatMessage({ id: "domain.registered_at" })]:
                    item.registered_at
                ...
                [formatMessage({ id: "domain.delete_at" })]: item.delete_at
                    ...
                    : "",
                [formatMessage({ id: "domain.force_delete_at" })]:
                    item.force_delete_at
                ...
                [formatMessage({
                    ...
                    ? moment(item.locked_by_registrant_at).format(
                          "DD.MM.Y HH:mm"
                      )
                    : "",
                [formatMessage({ id: "domain.registrant" })]: item.registrant
                    ? item.registrant.name
                    : "",
                [formatMessage({ id: "domain.tech_contacts" })]:
                    item.tech_contacts
                        ? item.tech_contacts
                              .map(
                                  (contact) =>
                              ...
                              .join("\n")
                        : "",
                [formatMessage({ id: "domain.admin_contacts" })]:
                    item.admin_contacts
                        ? item.admin_contacts
                              .map(
                                  (contact) =>
                ...
                [formatMessage({ id: "domain.statuses" })]: item.statuses
                    ? item.statuses
                          .map((status) =>
                              formatMessage({ id: `domain.status.${status}` })
                          )
                          .join("\n")
                    : "",
                [formatMessage({ id: "domain.reserved" })]: item.reserved
                    ? item.reserved
                    : "",
            }
        })
