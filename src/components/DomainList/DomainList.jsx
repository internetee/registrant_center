...
const DomainList = ({
    ...
    const [isGrid, setIsGrid] = useState(
        domainsIsGrid ? JSON.parse(domainsIsGrid) : true
    )
    ...
    const [perPage, setPerPage] = useState(
        domainsPerPage ? Number(domainsPerPage) : 24
    )
        ...
        if (domains.length) {
            const {
                ...
                (acc, domain) => ({
                    ...acc,
                    maxDate:
                        domain.valid_to > acc.maxDate
                            ? domain.valid_to
                            : acc.maxDate,
                    minDate:
                        domain.valid_to < acc.minDate
                            ? domain.valid_to
                            : acc.minDate,
                    registrants: {
                        ...acc.registrants,
                    ...
                    sortedDomains: [
                        ...acc.sortedDomains,
                        {
                            ...
                            statuses: domain.statuses.sort(
                                (a, b) =>
                                    domainStatuses[a].priority -
                                    domainStatuses[b].priority
                            ),
                        },
                    ],
                    statuses: {
            ...
            setStatuses(
                Object.values(allStatuses).sort(
                    (a, b) =>
                        domainStatuses[a].priority - domainStatuses[b].priority
                )
            )
            ...
            reg_list.some(
                (i, idx) =>
                    i.value == "all" &&
                    reg_list.unshift(reg_list.splice(idx, 1)[0])
            )
                ...
                domainsList
                    ...
                    .filter((domain) => {
                        if (queryValidToMin && queryValidToMax) {
                            return (
                                new Date(domain.valid_to).getTime() >=
                                    queryValidToMin &&
                                new Date(domain.valid_to).getTime() <=
                                    queryValidToMax
                            )
                    ...
                    .filter((domain) => {
                        if (
                            queryRegistrant.length &&
                            queryRegistrant !== "all"
                        ) {
                            return domain.registrant.ident.includes(
                                queryRegistrant
                            )
                        }
                        return true
                    ...
                    <Form className='form-filter' onSubmit={handleSubmit}>
                        <div className='form-filter--search'>
                            ...
                            <div className='form-filter--actions'>
                                ...
                                >
                                    <Icon
                                        className={classNames("action--list", {
                                            active: !isGrid,
                                        })}
                                        link
                                        name='th list'
                                    ...
                                    />
                                    <Icon
                                        className={classNames("action--grid", {
                                            active: isGrid,
                                        })}
                                        link
                                        name='th'
            ...
            {domains.length ? (
                <>
                    {paginatedDomains.length ? (
                        <>
                            ...
                            >
                                <div className='domains-grid--wrap'>
                                    ...
                                    >
                                        {paginatedDomains[activePage - 1].map(
                                            (domain) => (
                                                <DomainGridItem
                                                    key={domain.id}
                                                    domain={domain}
                        ...
                        <PageMessage
                            headerContent={
                                <FormattedMessage id='domains.search.message.title' />
                            }
                        />
                    )}
                </>
