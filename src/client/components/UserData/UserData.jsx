      ...
      const doc = {
        ...
        content: [
          ...chunk.map((item,i) => {
            let domainData = [
              item.transfer_code ? [
                {text: labels.transfer_code, bold: true},
                item.transfer_code] : [],
              item.registrar.name ? [
            ...
            const domainStatus = item.statuses ? item.statuses.map(status => [
            ...
            return [
              {text: item.name, style: 'header', pageBreak: i > 0 && 'before' },
              {
                ...
                layout: {
                  hLineWidth: (lineIndex, node) => (lineIndex === 0 || lineIndex === node.table.body.length) ? 1 : 1,
                  vLineWidth: () => 0,
                  hLineColor: () => '#eaebed',
                  vLineColor: () => '#eaebed',
                  paddingLeft: () => 10,
              ...
              {text: 'Kontaktid', style: 'subheader'},
              {
                ...
                table: {
                  ...
                  widths: ['30%', '35%', '35%'],
                  body: [
                    [{ text: 'Tüüp', bold: true }, { text: 'Nimi', bold: true }, { text: 'E-post', bold: true }],
                    ...registrantContacts,
                    ...techContacts,
                    ...adminContacts,
                  ]
              ...
              {text: labels.nameservers, style: 'subheader'},
              {
                ...
                table: {
                  ...
                  widths: ['30%', '35%', '35%'],
                  body: [
                    [{ text: 'Hostinimi', bold: true }, { text: 'ipv4', bold: true }, { text: 'ipv6', bold: true }],
                    ...domainNameservers
                  ]
                },
                layout: {
  ...
  render() {
    ...
    const userCSV = domains.map(item => {
      return {
        ...
        [labels.registrar]: item.registrar.name ? `${item.registrar.name} (${item.registrar.website})` : '',
        [labels.registered_at]: item.registered_at ? moment(item.registered_at).format('DD.MM.Y HH:mm') : '',
        [labels.valid_to]: item.valid_to ? moment(item.valid_to).format('DD.MM.Y HH:mm') : '',
        [labels.created_at]: item.created_at ? moment(item.created_at).format('DD.MM.Y HH:mm') : '',
        [labels.updated_at]: item.updated_at ? moment(item.updated_at).format('DD.MM.Y HH:mm') : '',
        [labels.period]: item.period ? `${item.period} (${item.period_unit})` : '',
        [labels.outzone_at]: item.outzone_at ? moment(item.outzone_at).format('DD.MM.Y HH:mm') : '',
        [labels.delete_at]: item.delete_at ? moment(item.delete_at).format('DD.MM.Y HH:mm') : '',
        [labels.force_delete_at]: item.force_delete_at ? moment(item.force_delete_at).format('DD.MM.Y HH:mm') : '',
            ...
            <Grid.Row centered columns='1'>
              <Grid.Column textAlign='center'>
                ...
                <Button.Group>
                  ...
                  <Button.Or text='või' />
                  <Button primary size='large' animated='fade' onClick={this.savePDF} loading={isLoadingPDF}>
                    <Button.Content visible><Icon name='file pdf' />PDF</Button.Content>
                    <Button.Content hidden>
                      <Icon name='download' />
                    </Button.Content>
                  </Button>
