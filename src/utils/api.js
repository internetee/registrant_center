...
export default {
    ...
    sendVerificationResponse: (name, token, action, type) => {
        return instance.post(
            `/api/confirms/${name}/${type}/${token}/${action}`,
            {
                credentials: "include",
                method: "POST",
                timeout: 4000,
    ...
    fetchDomains: (
        ...
        if (simplify) {
            return instance.get(
                `/api/domains?offset=${offset}&simple=true&tech=${tech}`
            )
        }
        if (!simplify) {
            return instance.get(
                `/api/domains?offset=${offset}&simple=false&tech=${tech}`
            )
        }
    ...
    setDomainRegistryLock: (uuid) =>
        instance.post(`/api/domains/${uuid}/registry_lock`),
