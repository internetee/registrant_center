...
export default {
    ...
    deleteDomainRegistryLock: async ({ params, session }, res) => {
        ...
        return handleResponse(
            () =>
                API(session).delete(
                    `/api/v1/registrant/domains/${uuid}/registry_lock`
                ),
            res
        )
    },
    ...
    getCompanies: async ({ query, session }, res) => {
        ...
        return handleResponse(
            () =>
                API(session, res).get(
                    `/api/v1/registrant/companies?offset=${offset}`
                ),
            res
        )
    },
                    ...
                    () =>
                        API(session, res).get(
                            `/api/v1/registrant/domains${
                                uuid
                                    ? `/${uuid}`
                                    : `?offset=${offset}&simple=true&tech=${tech}`
                            }`
                        ),
                    res
                )
                ...
                () =>
                    API(session, res).get(
                        `/api/v1/registrant/domains${
                            uuid
                                ? `/${uuid}`
                                : `?offset=${offset}&simple=false&tech=${tech}`
                        }`
                    ),
                res
            )
